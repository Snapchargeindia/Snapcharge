...
const runShiprocketCheckout = async ({
  ...
  try {
    const tokenRes = await fetch(
      "https://Snapcharge-backend-prod-env.eba-m6tqr9gn.eu-north-1.elasticbeanstalk.com/api/shiprocket/initiate",
      ...
      async (response) => {
        if (response?.status === "success") {
          ...
          const orderRes = await fetch(
            "https://Snapcharge-backend-prod-env.eba-m6tqr9gn.eu-north-1.elasticbeanstalk.com/api/shiprocket/create-order",
            {
              method: "POST",
    ...
    return (
      <div className="min-h-screen bg-[#FAEBD7] flex items-center justify-center px-4">
        <div className="bg-white px-8 py-6 rounded-2xl shadow-md text-center">
          ...
          <button onClick={() => navigate(-1)} className="px-6 py-2 rounded-full bg-[#7aa874] text-white text-sm font-medium hover:bg-[#6b9a65] transition">
            ← Go Back
          </button>
        </div>
      </div>
            ...
            <div className="mt-6">
              ...
              <div className="flex flex-wrap gap-3">
                {colorOptions.map((color) => (
                  ...
                  >
                    <span
                      className={`w-10 h-10 rounded-full border-2 ${color.targetId === product.id ? "border-black" : "border-[#d4d4d4]"}`}
            ...
            {[
              ...
              <button
                ...
                className={`pb-4 transition ${
                  activeTab === key ? "text-[#2f5a4d] border-b-2 border-[#7aa874]" : "hover:text-[#2f5a4d]"
                }`}
              >
                {label}
              </button>
