// src/LeatherCollectionMagsafe.jsx
import React from "react";
import { useNavigate } from "react-router-dom";
import { leatherCollectionMagsafeProducts } from "./leatherCollectionMagsafeData";
import { useCart } from "./CartContext";

const LeatherCollectionMagsafe = () => {
    ...
    <div className="min-h-screen bg-[#FAEBD7] pt-28 px-4 sm:px-8 lg:px-14 pb-16">
      ...
      <div className="flex justify-between items-center mb-10">
        ...
        <p className="text-[#6b7f6d] italic text-sm sm:text-base">
          {leatherCollectionMagsafeProducts.length} items
        </p>
      </div>

      {/* GRID */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {leatherCollectionMagsafeProducts.map((product) => (
          <div
            key={product.id}
            onClick={() =>
              navigate(`/leather-collection-magsafe/${product.id}`)
            }
...
export default LeatherCollectionMagsafe;