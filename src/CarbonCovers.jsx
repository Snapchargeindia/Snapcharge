// src/CarbonCovers.jsx
import React from "react";
import { useNavigate } from "react-router-dom";
import { carbonCoverProducts } from "./carbonCoverData";
    ...
    <div className="min-h-screen bg-[#FAEBD7] pt-28 px-4 sm:px-8 lg:px-14 pb-16">
      ...
      <div className="flex justify-between items-center mb-10">
        ...
        <p className="text-[#6b7f6d] italic text-sm sm:text-base">
          {carbonCoverProducts.length} items
        </p>
      </div>

      {/* GRID */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {carbonCoverProducts.map((product) => (
          <div
            key={product.id}
            onClick={() => navigate(`/carbon-covers/${product.id}`)}
